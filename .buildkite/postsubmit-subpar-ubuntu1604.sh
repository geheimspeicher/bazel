...
echo '--- Cloning'
git clone https://github.com/google/subpar .downstream-projects/subpar || exit $?
cd .downstream-projects/subpar
...
cd ../..
python3 .buildkite/failed_testlogs.py .downstream-projects/subpar/bep.json | while read logfile; do buildkite-agent artifact upload $logfile; done
