...
echo '--- Cleanup'
bazel clean --expunge
rm -rf bep.json .failed-test-logs .stashed-outputs skydoc
...
echo '--- Cloning'
git clone https://github.com/geheimspeicher/skydoc || exit $?
cd skydoc
...
cd ..
python3 .buildkite/failed_testlogs.py skydoc/bep.json | while read logfile; do buildkite-agent artifact upload $logfile; done
