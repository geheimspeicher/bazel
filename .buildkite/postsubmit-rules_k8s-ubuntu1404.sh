...
echo '--- Cloning'
git clone https://github.com/geheimspeicher/rules_k8s .downstream-projects/rules_k8s || exit $?
cd .downstream-projects/rules_k8s
...
cd ../..
python3 .buildkite/failed_testlogs.py .downstream-projects/rules_k8s/bep.json | while read logfile; do buildkite-agent artifact upload $logfile; done
