...
echo '--- Cleanup'
bazel clean --expunge
rm -rf bep.json .failed-test-logs .stashed-outputs bazel-watcher
...
echo '--- Cloning'
git clone https://github.com/geheimspeicher/bazel-watcher || exit $?
cd bazel-watcher
...
cd ..
python3 .buildkite/failed_testlogs.py bazel-watcher/bep.json | while read logfile; do buildkite-agent artifact upload $logfile; done
