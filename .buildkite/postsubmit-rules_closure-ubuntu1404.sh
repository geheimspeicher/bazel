...
echo '--- Cleanup'
bazel clean --expunge
rm -rf bep.json .failed-test-logs .stashed-outputs rules_closure
...
echo '--- Cloning'
git clone https://github.com/geheimspeicher/rules_closure || exit $?
cd rules_closure
...
cd ..
python3 .buildkite/failed_testlogs.py rules_closure/bep.json | while read logfile; do buildkite-agent artifact upload $logfile; done
