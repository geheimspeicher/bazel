...
echo '--- Cleanup'
bazel clean --expunge
rm -rf bep.json .failed-test-logs .stashed-outputs rules_jsonnet
...
echo '--- Cloning'
git clone https://github.com/geheimspeicher/rules_jsonnet || exit $?
cd rules_jsonnet
...
cd ..
python3 .buildkite/failed_testlogs.py rules_jsonnet/bep.json | while read logfile; do buildkite-agent artifact upload $logfile; done
