...
echo '--- Cleanup'
bazel clean --expunge
rm -rf bep.json .failed-test-logs .stashed-outputs rules_appengine
...
echo '--- Cloning'
git clone https://github.com/geheimspeicher/rules_appengine || exit $?
cd rules_appengine
...
cd ..
python3 .buildkite/failed_testlogs.py rules_appengine/bep.json | while read logfile; do buildkite-agent artifact upload $logfile; done
